...
async function recordHeadshot({ workspaceId, user }) {
  const profile = await AmbassadorProfile.findOne({
    workspaceId,
    userId: user._id,
    status: "active",
  }).lean();
  if (!profile) return;
  const config = await WorkspaceConfig.findOne({ workspaceId, key: "primary" })
    .select("ambassadorOnboarding")
    .lean();
  const ready = completeness(
    profile,
    user,
    config?.ambassadorOnboarding?.requiredFields,
  );
  await CrmActivity.create({
    ...
    title: user.avatarUrl
      ? "Ambassador headshot updated"
      : "Ambassador headshot removed",
    createdBy: user._id,
    metadata: {
      eventType: "ambassador.headshot.updated",
  ...
  if (ready.complete)
    await CrmActivity.create({
      ...
      type: "system",
      source: "crm",
      title: "Ambassador profile completed",
      createdBy: user._id,
      metadata: {
        eventType: "ambassador.profile.completed",
        ambassadorProfileId: profile._id,
      },
    });
}
async function recordProfileUpdate({ workspaceId, userId, previous, user }) {
  ...
  const before = completeness(
    profile,
    { ...previous, profileUpdatedAt: true },
    required,
  );
  const after = completeness(
    profile,
    { ...user, profileUpdatedAt: true },
    required,
  );
