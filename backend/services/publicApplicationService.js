...
function normalizeReferralInput(value){const raw=clean(value,1000);if(!raw)return"";try{const parsed=new URL(raw);const pathMatch=parsed.pathname.match(/\/ref\/([^/?#]+)/i);const candidate=pathMatch?.[1]||parsed.searchParams.get("ref")||parsed.searchParams.get("referral")||"";return clean(decodeURIComponent(candidate),80).toLowerCase()}catch{const pathMatch=raw.match(/(?:^|\/)ref\/([^/?#]+)/i);const candidate=pathMatch?.[1]||raw;try{return clean(decodeURIComponent(candidate),80).toLowerCase()}catch{return clean(candidate,80).toLowerCase()}}}
function publicConfig(config){const app=config?.publicApplication||{},labels=app.questionLabels||{},heading=!app.heading||app.heading==="Apply for coaching"?"Apply to Join a Program":app.heading,intro=!app.intro||app.intro==="Tell us where you are and where you want to go."?"Choose the program that fits your goals and tell us a little about where you are today.":app.intro;
return{enabled:app.enabled!==false,heading:clean(heading,240),intro:clean(intro,1200),confirmationMessage:clean(app.confirmationMessage||"Thank you. Your student application has been received. Our team will review it and follow up with next steps.",1000),heroImageUrl:clean(app.heroImageUrl,1000),questionLabels:{investingExperience:clean(labels.investingExperience||"Investing experience",160),currentSituation:clean(labels.currentSituation||"Current situation",160),goals:clean(labels.goals||"Goals",160),desiredStartTimeline:clean(labels.desiredStartTimeline||"Desired start timeline",160),message:clean(labels.message||"Anything else we should know?",160)},timelineOptions:(app.timelineOptions||[]).map(value=>clean(value,160)).filter(Boolean).slice(0,20),nextStepCta:{label:clean(app.nextStepCta?.label,120),url:clean(app.nextStepCta?.url,1000)},privacyUrl:clean(app.privacyUrl||"/privacy",1000),termsUrl:clean(app.termsUrl||"/terms",1000)}}
...
return{attribution:{referralCode:normalizeReferralInput(input.referralCode||tracked?.referralCode),trackedLinkToken:tracked?token:"",provider:clean(tracked?.provider,80),campaignId:tracked?.campaignId||null,contentId:clean(tracked?.contentId,255),utm:{...utm(input.utm),...(tracked?.utm||{})}},tracked}}
...
if(attribution.referralCode&&models.referralService?.resolvePromoterByCode){const promoter=await models.referralService.resolvePromoterByCode({workspaceId,referralCode:attribution.referralCode});if(!promoter)throw new Error("That referral code or link is not valid for this application. Check it and try again.")}
...
let opportunity=await models.SalesOpportunity.findOne({workspaceId,primaryContactId:contact._id,stageKey:{$nin:["won","lost"]},$or:[{coachingProgramId:program._id},{coachingProgramId:null}]});
if(!opportunity)opportunity=await models.SalesOpportunity.create({workspaceId,name:`${program.name} application — ${contact.name}`,stageKey:"new",primaryContactId:contact._id,campaignId:attribution.campaignId,ownerId:assignedUserId,applicationId:application._id,coachingProgramId:program._id,nextAction:"Review student application",notes:application.answers.message,leadLifecycle:{status:"application",statusAt:new Date()},leadAttribution:{source:"public_application",campaignId:attribution.campaignId,socialProvider:attribution.provider}});else{opportunity.applicationId=application._id;opportunity.coachingProgramId||=program._id;opportunity.campaignId||=attribution.campaignId;opportunity.ownerId||=assignedUserId;opportunity.nextAction="Review student application";opportunity.leadLifecycle={...(opportunity.leadLifecycle?.toObject?.()||opportunity.leadLifecycle||{}),status:"application",statusAt:new Date()};opportunity.leadAttribution={...(opportunity.leadAttribution?.toObject?.()||opportunity.leadAttribution||{}),source:opportunity.leadAttribution?.source||"public_application",campaignId:opportunity.leadAttribution?.campaignId||attribution.campaignId,socialProvider:opportunity.leadAttribution?.socialProvider||attribution.provider};await opportunity.save()}application.salesOpportunityId=opportunity._id;
...
return application}
module.exports={normalizeReferralInput,publicConfig,resolveApplicationContact,resolveAttribution,submit,utm};
