...
async function deliverInvitation({ invitation, token = crypto.randomBytes(32).toString("base64url"), workspaceName, invitedBy = "", replyToEmail = "" }, models) {
  ...
  for (const [key, message] of Object.entries({ firstName: "Recipient first name is required before sending.", displayName: "Recipient full name is required before sending.", role: "Invitation role is required before sending.", workspaceName: "Business/display name is required before sending.", invitedBy: "Inviter name is required before sending.", inviteLink: "Secure invitation link is required before sending." })) if (!String(vars[key] || "").trim()) throw Object.assign(new Error(message), { code: `INVITATION_${key.replace(/[A-Z]/g, (letter) => `_${letter}`).toUpperCase()}_REQUIRED` });
