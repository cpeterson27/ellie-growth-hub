...
async function createCheckout({ workspaceId, userId, input }) {
  ...
  if (existing) { const same = existing.kind === kind && existing.amountMinor === amountMinor && String(existing.contactId || "") === String(association.contactId || "") && String(existing.coachingProgramId || "") === String(association.program?._id || "") && String(existing.coachingApplicationId || "") === String(association.application?._id || "") && String(existing.salesOpportunityId || "") === String(association.opportunity?._id || ""); if (!same) throw Object.assign(new Error("Idempotency key was already used for a different payment request"), { code: "PAYMENT_IDEMPOTENCY_CONFLICT" }); return existing; }
  const hosted = await provider.createHostedCheckout(current.value.accessToken, { idempotencyKey, locationId: current.connection.externalLocationId, referenceId: idempotencyKey, description: String(input.description || association.program?.name || "Program payment").slice(0, 500), amountMinor, currency: "USD", redirectUrl: `${process.env.FRONTEND_URL?.split(",")[0]?.trim() || "http://localhost:5173"}/payment-complete` });
...
async function processSquareWebhook({ rawBody, signature }) {
  ...
  try {
    ...
    if (refundEvent?.id && refundEvent.payment_id) {
      ...
      await transaction.save(); await completeReceipt("processed"); return { processed: true };
    }
...
module.exports = { connectSquare, connectionStatus, createCheckout, disconnectSquare, getSettings, listTransactions, processSquareWebhook, refreshSquare, refund, safeConnection, updateSettings, validateAssociations };
