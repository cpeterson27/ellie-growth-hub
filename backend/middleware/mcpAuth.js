...
async function requireMcpAuth(req, res, next) {
  try {
    ...
    if (!match) return res.status(401).set("WWW-Authenticate", `Bearer realm="ellie-mcp", resource_metadata="${metadataUrl}"`).json({ error: "A valid Ellie MCP access token is required" });
