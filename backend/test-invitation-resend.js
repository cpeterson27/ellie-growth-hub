...
async function run() {
  const invitation = doc({ _id: "invite-1", workspaceId: "workspace-1", userId: "user-1", email: "pending@example.test", name: "Pending Person", roles: ["ambassador"], roleKey: "ambassador", subject: "[First name], join [Business name] as [Role]", body: "Hello [Full name]\nInvited by [Invited by]\n[Secure invitation button]", tokenHash: "original-hash", status: "pending", deliveryStatus: "sent", expiresAt: new Date(Date.now() + 60_000), invitedBy: "owner-1" });
  ...
  let providerCalls = 0, lastPayload;
  ...
  const models = {
    ...
    integrationHub: { execute: async (_provider, _action, payload) => { providerCalls += 1; lastPayload = payload; if (providerFailure) throw new Error("mock provider unavailable"); return { messageId: `mock-${providerCalls}` }; } },
  };
  ...
  for (const rendered of [lastPayload.subject, lastPayload.text, lastPayload.html]) assert(!/(\[(?:First name|Full name|Role|Business name|Secure invitation button|Invited by)\]|{{(?:firstName|displayName|role|workspaceName|inviteLink|invitedBy)}})/.test(rendered), "delivered content has no unresolved supported tokens");
  assert.equal(lastPayload.subject, "Pending, join Fixture Workspace as Brand Ambassador");
  assert.match(lastPayload.text, /Hello Pending Person/); assert.match(lastPayload.text, /Invited by Pending Person/); assert.match(lastPayload.text, /\/accept-invitation\//);
  assert.match(lastPayload.html, /<a href=.*accept-invitation/); assert.match(lastPayload.html, />Accept invitation<\/a>/);
