...
const PROVIDERS = new Set(
  require("../services/socialProviderConfig").SOCIAL_PROVIDERS,
);
  ...
  try {
    ...
    return res
      ...
      .json({
        error:
          status === 503
            ? "Meta deauthorization is unavailable"
...
router.get(
  ...
  async (req, res) => {
    ...
    try {
      return res.json({
        authorizationUrl: await socialOAuth.createAuthorizationRequest(
          id,
          req.auth,
        ),
      });
      ...
      return res
        .status(400)
        .json({ error: error.message || "Social OAuth is not configured" });
    }
  },
);
...
router.get("/:provider/oauth/callback", async (req, res) => {
  ...
  if (req.query.error)
    return res.redirect(
      frontendRedirect({
        social: id,
        status: "denied",
        message: "Authorization was declined or unavailable.",
      }),
    );
  ...
  try {
    await socialOAuth.exchangeCode(
      id,
    ...
    const safeError = socialOAuth.safeProviderError(
      error,
      "Social connection failed",
    );
    console.error(`[social oauth] ${id} callback failed: ${error.message}`, socialOAuth.safeProviderDiagnostic(error));
    return res.redirect(
      frontendRedirect({ social: id, status: "failed", message: safeError }),
    );
  }
});
...
router.patch(
  ...
  async (req, res) => {
    ...
    if (!id) return;
    try {
      const connection = await socialOAuth.selectAssets(
...
router.post(
  ...
  async (req, res) => {
    try {
      return res.json(await socialOAuth.refreshInstagram(req.auth.workspaceId));
    } catch {
      return res
        .status(400)
        .json({
          error:
            "Instagram authorization could not be refreshed. It must be unexpired and at least 24 hours old. Reconnect if necessary.",
        });
    }
  },
);
...
router.post(
  ...
  async (req, res, next) => {
    ...
    if (!id) return;
    try {
      return res.json(await socialOAuth.disconnect(req.auth.workspaceId, id));
    } catch (error) {
      return next(error);
    }
  },
