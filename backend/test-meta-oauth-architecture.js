...
const env = { META_APP_ID: "fb-app", META_APP_SECRET: "fb-secret", FACEBOOK_LOGIN_CONFIG_ID: "123456789",
  ...
  META_GRAPH_API_VERSION: "v26.0", FRONTEND_URL: "https://elliescoaching.com",
  INTEGRATION_CREDENTIAL_ENCRYPTION_KEY: crypto.randomBytes(32).toString("base64") };
Object.assign(process.env, env);
delete process.env.META_OAUTH_SCOPES; delete process.env.INSTAGRAM_OAUTH_SCOPES;
const auth = { workspaceId: "workspace-a", user: { _id: "owner-a" } };
const url = provider => new URL(oauth.authorizationUrl(provider, auth));
const fb = url("meta"), ig = url("instagram");
...
assert(!fb.searchParams.has("scope")); assert(!fb.searchParams.has("client_secret"));
...
assert(!ig.searchParams.has("config_id")); assert(!ig.searchParams.get("scope").includes("pages_"));
for (const [provider, value] of [["meta", fb], ["instagram", ig]]) {
  assert.equal(value.searchParams.get("redirect_uri"), env[provider === "meta" ? "META_REDIRECT_URI" : "INSTAGRAM_REDIRECT_URI"]);
  const state = value.searchParams.get("state");
  assert.equal(oauth.verifyState(state, provider).workspaceId, auth.workspaceId);
  ...
  assert.equal(oauth.verifyState(state, provider === "meta" ? "instagram" : "meta"), null);
}
for (const [key, value] of [["FACEBOOK_LOGIN_CONFIG_ID", ""], ["FACEBOOK_LOGIN_CONFIG_ID", "abc"], ["META_GRAPH_API_VERSION", "v26.0/bad"], ["META_REDIRECT_URI", "https://example.test/wrong"], ["META_OAUTH_SCOPES", "pages_manage_postspages_manage_engagement"], ["META_OAUTH_SCOPES", "instagram_basicinstagram_manage_messages"]]) {
  const old = process.env[key]; process.env[key] = value;
  assert.throws(() => url("meta")); assert.equal(oauth.configured("meta"), false);
  if (old === undefined) delete process.env[key]; else process.env[key] = old;
}
for (const value of ["", "instagram_basic", "instagram_business_basicinstagram_business_manage_messages"]) {
  process.env.INSTAGRAM_OAUTH_SCOPES = value; assert.throws(() => url("instagram"));
}
delete process.env.INSTAGRAM_OAUTH_SCOPES;
delete process.env.FACEBOOK_LOGIN_CONFIG_ID;
assert.equal(oauth.configured("instagram"), true, "Instagram must not require Facebook Business configuration");
process.env.FACEBOOK_LOGIN_CONFIG_ID = env.FACEBOOK_LOGIN_CONFIG_ID;
delete process.env.INSTAGRAM_APP_SECRET;
assert.equal(oauth.configured("instagram"), false);
assert.equal(oauth.configured("meta"), true, "Facebook must not require Instagram credentials");
process.env.INSTAGRAM_APP_SECRET = env.INSTAGRAM_APP_SECRET;
assert.deepEqual(settings.scopes("meta", "pages_show_list, pages_manage_posts pages_show_list"), ["pages_show_list", "pages_manage_posts"]);
assert.deepEqual(Connection.schema.path("provider").enumValues, [...settings.SOCIAL_PROVIDERS]);
...
async function run() {
  try {
    ...
    Connection.findOneAndUpdate = async (filter, update) => {
      ...
      row.toObject = () => ({ ...row }); row.save = async () => { stored.set(keyFor(filter), row); };
      stored.set(keyFor(filter), row); return row;
    };
    Connection.findOne = filter => query(stored.get(keyFor(filter)) || null);
    axios.get = async (address, options) => {
      calls.push(address);
      const u = new URL(address);
      if (!address.endsWith("/access_token")) assert(u.pathname.startsWith("/v26.0/"));
      if (u.hostname === "graph.facebook.com") {
        if (address.endsWith("/oauth/access_token")) return { data: { access_token: options.params.grant_type ? "fb-long" : "fb-short", expires_in: 3600 } };
    ...
    axios.post = async (address, body) => {
      calls.push(address);
      if (address === "https://api.instagram.com/oauth/access_token") { assert.equal(new URLSearchParams(body).get("client_id"), "ig-app"); return { data: { access_token: "ig-short", user_id: "ig-user" } }; }
      assert(address.includes("/v26.0/") && address.endsWith("/subscribed_apps"));
      return { data: { success: true } };
    };
    axios.delete = async address => { calls.push(address); assert(address.includes("graph.facebook.com/v26.0/me/permissions")); return { data: { success: true } }; };
    const callback = routes.stack.find(layer => layer.route?.path === "/:provider/oauth/callback").route.stack.at(-1).handle;
    for (const provider of ["meta", "instagram"]) {
      const state = url(provider).searchParams.get("state");
      let redirect;
      await callback({ params: { provider }, query: { code: "mock-code", state } }, { redirect(value) { redirect = value; } });
      assert.equal(new URL(redirect).searchParams.get("status"), "connected");
      const row = stored.get(`workspace-a:${provider}`);
      assert(row.credentialsEncrypted); assert(!JSON.stringify(row.credentialsEncrypted).includes(provider === "meta" ? "fb-long" : "ig-long"));
      assert.equal(decryptCredentials(row.credentialsEncrypted).accessToken, provider === "meta" ? "fb-long" : "ig-long");
      assert.equal((await oauth.status("workspace-a", provider)).connected, true);
      assert.equal((await oauth.status("workspace-b", provider)).connected, false);
      assert(!("credentialsEncrypted" in await oauth.status("workspace-a", provider)));
      const asset = provider === "meta" ? "page-a" : "ig-user";
      await assert.rejects(oauth.selectAssets("workspace-a", provider, ["unowned"]), /Choose only assets/);
    ...
    await assert.rejects(oauth.exchangeCode("instagram", "code", fb.searchParams.get("state")), /invalid/);
    for (const provider of ["meta", "instagram"]) {
      await oauth.disconnect("workspace-a", provider);
      assert.equal((await oauth.status("workspace-a", provider)).connected, false);
