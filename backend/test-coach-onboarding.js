...
async function existingUserIsReused() {
  ...
  const result = await service.onboardCoach(
    {
      workspaceId: "w1",
      actorUserId: "owner1",
      name: "Ada",
  ...
  assert.deepEqual(membership.roles, ["member", "coach"]);
  assert.equal(result.coachProfile.status, "active");
...
async function invitationCreatesInactiveProfileWithoutRealEmail() {
  ...
  assert.equal(providerCalls, 0, "creating the draft never sends email");
  assert.equal(invitation.deliveryStatus, "pending");
  assert.equal(result.membership.status, "invited");
  assert.equal(result.coachProfile.status, "inactive");
  const delivery = await service.sendInvitation(
    { workspaceId: "w1", invitationId: "i1" },
    {
      ...models,
      WorkspaceInvitation: {
  ...
  assert.equal(providerCalls, 1, "explicit send uses only the mocked provider");
  assert.equal(invitation.status, "pending");
  assert(invitation.renderedBody.includes("Fixture Workspace"));
}

async function crossWorkspaceProgramsFailClosed() {
  await assert.rejects(
    () =>
      service.onboardCoach(
        {
          workspaceId: "w1",
          actorUserId: "owner1",
          name: "Coach",
    ...
    (error) => error.code === "PROGRAM_WORKSPACE_MISMATCH",
  );
}
...
async function invitationAcceptanceActivatesCoach() {
  ...
  await service.acceptInvitation(
    { token, password: "a-secure-password", name: "Coach Name" },
    {
      ...
      CoachProfile: {
        findOneAndUpdate: async (_filter, update) => {
          profileStatus = update.$set.status;
        },
      },
    },
  );
  ...
  assert.equal(user.name, "Coach Name");
}
...
function roleSafety() {
  ...
  assert.throws(
    ...
    (error) => error.code === "OWNER_LOCKOUT_BLOCKED",
  );
  assert.doesNotThrow(() =>
    validateMembershipChange({
      currentRoles: ["owner"],
...
Promise.resolve()
  .then(existingUserIsReused)
  .then(invitationCreatesInactiveProfileWithoutRealEmail)
  .then(crossWorkspaceProgramsFailClosed)
  .then(invitationAcceptanceActivatesCoach)
  .then(roleSafety)
  .then(() =>
    console.log(
      "Coach invitation, profile linkage, workspace validation, duplicate reuse, and owner safety checks passed.",
    ),
  )
  .catch((error) => {
    console.error(error);
