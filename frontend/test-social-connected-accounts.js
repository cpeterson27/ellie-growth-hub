...
assert.equal(channelDefinitions.find(row => row.provider === "meta").name, "Facebook Page and linked Instagram");
assert.match(channelDefinitions.find(row => row.provider === "instagram").description, /Alternative: sign in directly/);
...
assert.match(component, /Linked Instagram:/);
assert.match(component, /direct-login method uses the Instagram Business permission flow/);
