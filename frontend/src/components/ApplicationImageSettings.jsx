...
export default function ApplicationImageSettings() {
  ...
  return <section className="application-image-settings"><div><p className="page-eyebrow">Student application</p><h3>Application image</h3><p>Choose the image students see when the application opens. This is optional — leave it empty and your Website &amp; Brand "Website header logo" is used automatically instead.</p></div><div className="application-image-settings__control"><div className="application-image-settings__preview">{config.heroImageUrl ? <img src={config.heroImageUrl} alt="Current student application" /> : <span>No custom image — using your website header logo</span>}</div><div><label className="website-upload-button">{busy ? "Uploading…" : config.heroImageUrl ? "Replace image" : "Upload image"}<input disabled={busy} type="file" accept="image/png,image/jpeg,image/webp" onChange={(event) => upload(event.target.files?.[0])}/></label>{config.heroImageUrl ? <Button variant="secondary" disabled={busy} onClick={remove}>Remove (use website logo instead)</Button> : null}<small>PNG, JPG, or WEBP up to 5 MB.</small></div></div><p aria-live="polite">{message}</p></section>;
}
