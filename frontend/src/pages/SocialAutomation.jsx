  ...
  return <main className="social-automation">
    ...
    <section className="social-grid">
      <form className="social-panel" onSubmit={save}><h2>New supported trigger</h2>
        ...
        <label>Platform<select disabled={Boolean(editingId)} value={form.provider} onChange={(e) => setForm({ ...form, provider: e.target.value, triggerType: e.target.value === "instagram" ? "comment_keyword" : "comment_keyword" })}><option value="instagram">Instagram</option><option value="facebook">Facebook</option></select></label>
        <label>Connected asset<select disabled={Boolean(editingId)} required value={form.assetId} onChange={(e) => setForm({ ...form, assetId: e.target.value })}><option value="">Select an account/page</option>{assets.map((asset) => <option key={asset.id} value={asset.id}>{asset.name || asset.username || asset.id}</option>)}</select></label>
        <label>Trigger<select disabled={Boolean(editingId)} value={form.triggerType} onChange={(e) => setForm({ ...form, triggerType: e.target.value })}><option value="comment_keyword">Comment contains keyword</option><option value="comment_any">Any comment</option><option value="dm_keyword">DM contains keyword</option>{form.provider === "instagram" ? <option value="story_reply">Story reply</option> : null}</select></label>
