...
export default function SocialAutomation() {
  ...
  return <main className="social-automation">
    ...
    <section className="social-grid social-grid--metrics">
      ...
      <article><strong>{assets.length}</strong><span>Connected accounts</span></article>
    </section>
    ...
    <section className="social-grid">
      <form className="social-panel" onSubmit={save}><h2>Build an interaction workflow</h2>
        <label>Name<input required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} /></label>
        <h3>WHEN · choose an interaction</h3><label>Platform<select disabled={Boolean(editingId)} value={form.provider} onChange={(e) => setForm({ ...form, provider: e.target.value, triggerType: "comment_keyword", assetId: "" })}><option value="instagram">Instagram</option><option value="facebook">Facebook</option></select></label>
        <label>Connected account<select disabled={Boolean(editingId)} required value={form.assetId} onChange={(e) => setForm({ ...form, assetId: e.target.value })}><option value="">Select an account/page</option>{assets.map((asset) => <option key={asset.id} value={asset.id}>{asset.name || asset.username || "Account"}</option>)}</select></label>
        <label>When someone<select disabled={Boolean(editingId)} value={form.triggerType} onChange={(e) => setForm({ ...form, triggerType: e.target.value })}>{categories.map(category => <optgroup label={category.title} key={category.title}>{category.triggers.map(([value, label]) => <option value={value} key={value}>{label}</option>)}</optgroup>)}</select></label>
        ...
        <h3>IF · match a keyword</h3><label>Keywords <small>(comma separated)</small><input placeholder="DEAL" value={form.keywords} onChange={(e) => setForm({ ...form, keywords: e.target.value })} disabled={form.triggerType !== "comment_keyword" && form.triggerType !== "dm_keyword"} /></label>
        <h3>THEN · organize and respond</h3><p>Find or create the contact, record the interaction, and optionally add tags or a permitted reply.</p>{contextOnly && <p>This interaction records attribution and CRM activity only. It does not authorize an automatic message.</p>}<label>Initial response<textarea disabled={contextOnly} maxLength={2000} rows="4" value={form.responseTemplate} onChange={(e) => setForm({ ...form, responseTemplate: e.target.value })} /></label>
        <label>Link button text<input value={form.ctaLabel} onChange={(e) => setForm({ ...form, ctaLabel: e.target.value })} placeholder="Watch the underwriting video" /></label>
        <label>Resource or application link<input type="url" value={form.ctaDestination} onChange={(e) => setForm({ ...form, ctaDestination: e.target.value })} placeholder="https://elliescoaching.com/apply" /></label>
    ...
    <section className="social-panel"><h2>Recent social leads</h2>{leads.length ? <div className="lead-table">{leads.map((identity) => <article key={identity._id}><div><strong>{identity.contactId?.name || identity.displayName || identity.username}</strong><span>{identity.provider}{identity.username ? ` · @${identity.username}` : ""}</span></div><div><span>{identity.contactId?.socialAttribution?.latest?.contentId ? "Source post recorded" : "Conversation source"}</span><small>{new Date(identity.lastActivityAt).toLocaleString()}</small></div></article>)}</div> : <p>No social leads yet. Contacts appear here when your connected accounts receive eligible interactions.</p>}</section>
