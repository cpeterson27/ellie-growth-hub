...
export function CoachDashboard() {
  ...
  return <div className="coaching-page"><PageHeader eyebrow="Coach Portal" title={`Welcome${profile?.displayName ? `, ${profile.displayName}` : ""}`} description="Your authorized students and assignments. Growth Operator only shows coaching records assigned to you." />
...
function CalendarConnection({ onChanged }) {
  ...
  const disconnect = async () => { if (!window.confirm("Disconnect your Google Calendar? Existing Growth Operator sessions remain recorded.")) return; setBusy(true); try { await disconnectCoachCalendar(); await load(); } catch (reason) { setError(message(reason)); } finally { setBusy(false); } };
