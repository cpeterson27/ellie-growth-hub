  ...
  return (
    <div className="ellie-site">
      <header className="ellie-site__nav">
        <Link className="ellie-wordmark" to="/" aria-label="Lead Porch home"><span>G</span><strong>Lead Porch</strong></Link>
        ...
        <div className="ellie-site__nav-actions">
          {loading ? <span className="text-link" aria-live="polite">Checking session…</span> : signedIn ? <>
            ...
            <Link className="site-button site-button--small" to="/dashboard">Open Lead Porch</Link>
          </> : <>
            <Link className="text-link" to="/login">Client login</Link>
            <a className="site-button site-button--small" href="#pricing">Start Lead Porch</a>
          </>}
        </div>
      </header>
      ...
      <main>
        <section className="ellie-hero">
          <div className="ellie-hero__copy">
            ...
            <p className="ellie-hero__lead">Lead Porch brings your CRM, campaigns, outreach, events, and follow-up into one private command center—guided by an AI operator that understands the work.</p>
            <div className="ellie-hero__actions"><a className="site-button" href="#pricing">Explore Lead Porch <FiArrowRight /></a><a className="site-button site-button--ghost" href="#how-it-works">See how it works</a></div>
          ...
          <div className="ellie-hero__visual" aria-label="Lead Porch campaign overview preview">
        ...
        <section className="ellie-workflow" id="how-it-works">
          <div className="ellie-workflow__intro"><p className="site-kicker">Built around responsible momentum</p><h2>AI prepares the work.<br /><em>You stay in command.</em></h2><p>Lead Porch is designed to make a small team more capable without turning judgment over to automation.</p></div>
          <div className="ellie-workflow__steps">
            <article><span>01</span><div><h3>Define the goal</h3><p>Create an event or offer campaign and tell Lead Porch who it is for.</p></div></article>
            ...
            <article><span>03</span><div><h3>Approve the message</h3><p>Edit one master template while Lead Porch prepares relevant personalization.</p></div></article>
        ...
        <section className="ellie-pricing" id="pricing">
          <div><p className="site-kicker">Founding customer access</p><h2>Build your growth system with Lead Porch.</h2><p>Early customers receive a private workspace, guided setup, campaign configuration, and direct onboarding while self-service subscriptions are prepared.</p><ul><li><FiCheck /> Private owner account and workspace</li><li><FiCheck /> CRM and campaign setup</li><li><FiCheck /> Integrations and outreach configuration</li><li><FiCheck /> Team members added by invitation</li></ul></div>
          <article><p>Early access</p><h3>Founding workspace</h3><span>Custom onboarding</span><a className="site-button" href="mailto:team@elliescoaching.com?subject=Growth%20Operator%20Founding%20Access">Request founding access <FiArrowRight /></a><small>{signedIn ? <>Your session is active. <Link to="/dashboard">Open Lead Porch</Link></> : <>Already a client? <Link to="/login">Sign in to Lead Porch</Link></>}</small></article>
        </section>
      </main>

      <footer className="ellie-site__footer"><Link className="ellie-wordmark" to="/"><span>G</span><strong>Lead Porch</strong></Link><p>Intelligent growth, operated with intention.</p><div><Link to={signedIn ? "/dashboard" : "/login"}>{signedIn ? "Open Lead Porch" : "Client login"}</Link><a href="mailto:team@elliescoaching.com">Contact</a></div></footer>
    </div>
  );
}
