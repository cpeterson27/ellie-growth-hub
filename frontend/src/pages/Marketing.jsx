...
import { fetchMarketingFeed } from "../services/api.js";
...
export default function Marketing() {
  ...
  const loadFeed = async () => {
    try {
      setLoading(true);
      const feed = await fetchMarketingFeed();
      setActions(feed.actions || []);
      setHistory(feed.history || []);
      setError("");
    } catch (err) {
      setError(err.response?.data?.error || "Growth recommendations are unavailable right now.");
  ...
  return <div className="page-dashboard marketing-page">
    <header className="marketing-hero"><div><p className="marketing-eyebrow">Distribution center</p><h1 className="page-title">Marketing</h1><p>Plan channel-specific messages, keep approved post drafts organized, and turn campaign opportunities into action.</p></div><Button onClick={() => setCompose(true)}>Create social post</Button></header>
    {error ? <p className="form-error">{error}</p> : null}
    <section className="marketing-channels">{channels.map((channel) => <DashboardCard key={channel} title={channel}><p className="marketing-channel__status">Configuration required</p><p>Connect the account credentials in your environment before publishing. Drafting works now; this page will not publish anything automatically.</p><Button variant="outline" size="sm" onClick={() => { setDraft({ ...draft, channel }); setCompose(true); }}>Create draft</Button></DashboardCard>)}</section>
    <section className="marketing-grid"><DashboardCard title="Growth recommendations">{loading ? <p>Loading recommendations…</p> : actions.length ? <div className="marketing-recommendations">{actions.map((action) => <article key={action.opportunityId} className="marketing-recommendation"><span className="label-pill">Priority {action.priority || "—"}</span><h3>{action.organizationName}</h3><p>{action.recommendedAction}</p>{action.reasons?.length ? <ul>{action.reasons.map((reason) => <li key={reason}>{reason}</li>)}</ul> : null}</article>)}</div> : <p className="marketing-empty">No live recommendations are available yet. Use Jarvis or create a campaign plan to begin.</p>}</DashboardCard><DashboardCard title="Post drafts">{posts.length ? <div className="marketing-drafts">{posts.map((post) => <article key={post.id}><span>{post.channel}</span><h3>{post.title || "Untitled post"}</h3><p>{post.copy}</p><small>Draft — not published</small></article>)}</div> : <div className="table-state table-state--empty">No social drafts yet. Create one for any connected channel.</div>}</DashboardCard></section>
    <DashboardCard title="Recent Growth Activity">{history.length ? <div className="marketing-history">{history.map((item) => <div key={item.opportunityId}><strong>{item.organizationName}</strong><span>{item.recommendedAction}</span></div>)}</div> : <p>No growth activity has been recorded yet.</p>}</DashboardCard>
