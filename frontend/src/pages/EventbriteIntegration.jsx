    ...
    <div className="page-dashboard integrations-page">
      ...
      <section className="eventbrite-setup-grid">
        ...
        >
          ...
          <div className="eventbrite-health-list">
            <div>
              ...
              <p><strong>Connect Eventbrite account</strong><small>{connection?.connected ? `${connection.accountEmail || "Authorized account"} is connected.` : connection?.configured ? "Client action: connect Eventbrite with OAuth." : "Developer action first: configure Ellie’s Eventbrite app credentials."}</small></p>
            </div>
            <div>
              <span className={`eventbrite-step-number ${webhookVerified ? "is-ready" : ""}`}>{webhookVerified ? "✓" : "2"}</span>
              <p><strong>Verify automatic updates</strong><small>{webhookVerified ? `Eventbrite last checked in ${formatDateTime(webhook.lastReceivedAt)}.` : webhook?.configured ? "Click Test in Eventbrite or wait for the first real event update." : "Developer action: configure Ellie’s webhook receiver token."}</small></p>
            </div>
            <div>
              <span className={`eventbrite-step-number ${latestSync ? "is-ready" : ""}`}>{latestSync ? "✓" : "3"}</span>
              <p><strong>Confirm reporting sync</strong><small>{latestSync ? `Ellie last refreshed event data ${formatDateTime(latestSync)}.` : "Open or import a connected Eventbrite event so Ellie can pull registrations, revenue, and check-ins."}</small></p>
            </div>
          </div>
        ...
        <DashboardCard title="What this integration does">
          <div className="integration-role-list">
            <article><strong>Imports events</strong><p>Add existing Eventbrite listings to Ellie so campaigns can be planned around them.</p></article>
            <article><strong>Publishes drafts</strong><p>Create an Eventbrite draft from an Ellie event only after the user confirms it.</p></article>
            <article><strong>Refreshes reporting</strong><p>Orders, attendees, check-ins, ticket changes, and event edits update Ellie automatically after setup is verified.</p></article>
          </div>
        </DashboardCard>
      </section>
      ...
      <DashboardCard title="Developer setup values">
        <div className="eventbrite-config-list">
          <div><span className={connection?.configured ? "is-ready" : ""}>{connection?.configured ? "✓" : "!"}</span><p><strong>OAuth app credentials</strong><small>{connection?.configured ? "Configured in Ellie’s backend environment." : "Set EVENTBRITE_CLIENT_ID, EVENTBRITE_CLIENT_SECRET, EVENTBRITE_REDIRECT_URI, and INTEGRATION_CREDENTIAL_ENCRYPTION_KEY."}</small></p></div>
          <div><span className={webhook?.configured ? "is-ready" : ""}>{webhook?.configured ? "✓" : "!"}</span><p><strong>Webhook receiver token</strong><small>{webhook?.configured ? "Configured in Ellie’s backend environment." : "Set EVENTBRITE_WEBHOOK_TOKEN in the backend environment."}</small></p></div>
