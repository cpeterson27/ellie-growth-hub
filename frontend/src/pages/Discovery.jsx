  ...
  return (
    <div className="page-dashboard discovery-page">
      ...
      <DashboardCard title="Filters" className="apollo-filter-panel" action={<span className="apollo-filter-count">{searchMode === "people" ? "People" : "Companies"}</span>}>
        ...
        <div className="apollo-search-actions"><div><strong>{searchMode === "people" && peopleSearchRunsInApollo ? "Ready to search Apollo" : `Ready to discover ${searchMode}`}</strong><span>{searchMode === "people" && peopleSearchRunsInApollo ? "Uses the filters below and opens Apollo’s People page." : "Returns new Apollo prospects here for review."}</span></div><Button loading={searchingApollo} onClick={() => searchMode === "people" && peopleSearchRunsInApollo ? openApolloPeopleSearch() : runApolloSearch(1)}>{searchMode === "people" ? (peopleSearchRunsInApollo ? "Open filtered search in Apollo" : "Find new people") : "Find new organizations"}</Button></div>
