...
const JOB_TITLE_SUGGESTIONS = ["Acquisitions Manager", "Director of Acquisitions", "Vice President of Acquisitions", "Underwriter", "Senior Underwriter", "Managing Partner", "General Partner", "Operating Partner", "Owner", "Founder", "Co-Founder", "Principal", "Managing Director", "Real Estate Investor", "Real Estate Developer", "Asset Manager", "Portfolio Manager", "Property Manager", "Real Estate Broker", "Mortgage Broker"];
    ...
    <div className="page-dashboard discovery-page">
      ...
      <DashboardCard title="Filters" className="apollo-filter-panel" action={<span className="apollo-filter-count">{searchMode === "people" ? "People" : "Companies"}</span>}>
        ...
        <div className="apollo-target-topline">
          ...
          <label>Search for<select value={searchMode} onChange={(event) => setSearchMode(event.target.value)}><option value="organizations">Organizations</option><option value="people">People</option></select><small>Both options discover net-new prospects from Apollo.</small></label>
        </div>
        {searchMode === "people" && !peopleApiAvailable ? <div className="apollo-capability-notice" role="status"><div><strong>Search destination: Apollo People Search</strong><span>Build the filters here, then use the always-visible button below to open the completed search in Apollo. These are new Apollo prospects—not contacts already stored in Ellie.</span><small>After an Apollo API upgrade, the same net-new Apollo results can be displayed in Ellie for review and import.</small></div></div> : null}
        <div className="apollo-target-grid">
          ...
          {searchMode === "people" ? <label>Job titles<TypeaheadInput label="Job titles" value={target.titles} onChange={(titles) => setTarget({ ...target, titles })} suggestions={JOB_TITLE_SUGGESTIONS} placeholder="Start typing: Acquisitions Manager" /><small>Choose one or more titles. You can also keep a custom title.</small></label> : null}
      ...
      <DashboardCard title="Search activity" className="apollo-history-panel" action={<span>Last {apolloHistory.length}</span>}>
        ...
        {apolloHistory.length ? <div className="apollo-history-table"><div className="apollo-history-row apollo-history-row--header"><span>Date</span><span>Search</span><span>Mode</span><span>Status</span><span>Matches</span><span>Time</span></div>{apolloHistory.slice(0, 5).map((run) => <div className="apollo-history-item" key={run._id}><div className="apollo-history-row"><span>{new Date(run.createdAt).toLocaleString()}</span><span>{run.templateName || "Custom search"}</span><span>{run.mode}</span><span className={`is-${run.status}`}>{run.status}</span><span>{(run.totalMatches || 0).toLocaleString()}</span><span>{((run.durationMs || 0) / 1000).toFixed(1)}s</span></div>{run.status === "error" ? <div className="apollo-history-error"><strong>{run.errorCode === "forbidden" ? "Apollo plan blocked API search" : "Why this search failed"}</strong><p>{run.errorMessage || "Apollo did not provide an error description for this search."}</p><span>{run.errorCode === "forbidden" ? "Next step: reopen this template and choose Open in Apollo. Upgrade Apollo only if you want results returned automatically inside Ellie." : "Next step: review the filters and Apollo connection, then retry the search."}</span>{run.errorCode ? <small>Error code: {run.errorCode}</small> : null}</div> : null}</div>)}</div> : <div className="table-state table-state--empty">Search history will appear after the first Apollo search.</div>}
      </DashboardCard>
      </div>
      </div>
