    ...
    <div className="page-dashboard discovery-page">
      ...
      <DashboardCard title="Filters" className="apollo-filter-panel" action={<span className="apollo-filter-count">{searchMode === "people" ? "People" : "Companies"}</span>}>
        ...
        <div className="apollo-target-topline">
          ...
          <label>Find<select value={searchMode} onChange={(event) => setSearchMode(event.target.value)}><option value="organizations">Companies inside Ellie</option><option value="people">People in Apollo</option></select><small>{searchMode === "people" ? (peopleApiAvailable ? "Eligible searches can return results inside Ellie." : "Ellie builds the filters; Apollo displays the results.") : "Company results return inside Ellie."}</small></label>
        </div>
        {searchMode === "people" && !peopleApiAvailable ? <div className="apollo-capability-notice" role="status"><div><strong>People Search opens in Apollo on your current plan</strong><span>Ellie will build and save the exact titles, locations, company keywords, employee size, and email-status filters. Clicking the final button opens those filters in Apollo’s People page; people results will not appear inside Ellie until the Apollo plan and API key include People Search API access.</span><small>No search is performed until you click Open people search in Apollo.</small></div></div> : null}
        ...
        <div className="apollo-search-actions"><Button loading={searchingApollo} onClick={() => searchMode === "people" && peopleSearchRunsInApollo ? openApolloPeopleSearch() : runApolloSearch(1)}>{searchMode === "people" ? (peopleSearchRunsInApollo ? "Open people search in Apollo" : "Find matching people in Ellie") : "Find matching companies in Ellie"}</Button></div>
